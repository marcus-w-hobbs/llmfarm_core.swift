...
public class LLaMa: LLMBase {
  ...
  public override func llm_load_model(
    path: String = "", contextParams: ModelAndContextParams = .default
  ) throws -> Bool {
  ...
  public override func load_state() {
      ...
      && FileManager.default.fileExists(atPath: self.contextParams.state_dump_path)
    {
      ...
      llama_state_load_file(
        self.context, self.contextParams.state_dump_path, tokens_tmp.mutPtr, 4096, &tokens_count)
      if tokens_count > 0 {
  ...
  public override func save_state() {
      ...
      && self.context != nil
    {
      ...
      llama_state_save_file(
        self.context, self.contextParams.state_dump_path, self.outputRepeatTokens,
        self.outputRepeatTokens.count)
    }
  }

  private func retain_new_self_ptr() {
    LLaMa_obj = Unmanaged<LLaMa>.fromOpaque(Unmanaged.passRetained(self).toOpaque())
      .takeRetainedValue()
  }
  ...
  public override func load_grammar(_ path: String) throws {}
  ...
  public override func kv_shift() throws {
    ...
    llama_kv_cache_seq_rm(
      context, 0, self.sampleParams.repeat_last_n, self.sampleParams.repeat_last_n + n_discard)
    llama_kv_cache_seq_add(
      context, 0, self.sampleParams.repeat_last_n + n_discard, self.nPast, -n_discard)
  ...
  private func token_to_piece(token: Int32) -> [CChar] {
    ...
    let nTokens = llama_token_to_piece(
      model, token, result, 8, 0, /*true*/ self.contextParams.parse_special_tokens)
    ...
    if nTokens < 0 {
      ...
      let nNewTokens = llama_token_to_piece(
        model, token, newResult, -nTokens, 0, /*true*/ self.contextParams.parse_special_tokens)
  ...
  public override func llm_tokenize(_ input: String, add_bos: Bool?, parse_special: Bool?)
    -> [ModelToken]
  {
    if input.count == 0 {
      return []
    ...
    var embeddings: [llama_token] = [llama_token](repeating: llama_token(), count: input.utf8.count)
    let n: Int32 = llama_tokenize(
      self.model, input, Int32(input.utf8.count), &embeddings, n_tokens,
