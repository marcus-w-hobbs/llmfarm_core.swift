...
struct Persona {
  ...
  private func _sanitizeIdentifier(_ name: String) -> String {
    return
      name
      .trimmingCharacters(in: .whitespacesAndNewlines)  // Handle leading/trailing whitespace
      .components(separatedBy: .whitespacesAndNewlines)  // Split on any whitespace character
      .filter { !$0.isEmpty }  // Remove empty components
      .joined(separator: "_")  // Join with underscores
      .replacingOccurrences(of: #"[^\w_]"#, with: "", options: .regularExpression)  // Remove non-word chars except underscore
  }
}

struct Personas {

  static let User = Persona(name: "User", systemPrompt: "")  // User is needed for Turns

  static let EdmondOtis01 =
    Persona(
      name: "Edmond Otis",
      systemPrompt:
        """
  ...
  static let Kierkegaard01 = Persona(
    name: "Kierkegaard",
    systemPrompt:
      """
  ...
  static let Nietzsche01 = Persona(
    name: "Nietszche",
    systemPrompt:
      """
  ...
  static let AncientTextWiseFriend = Persona(
    name: "Wise Friend",
    systemPrompt:
      """
  ...
  static let AncientTextPracticalGuide = Persona(
  ...
  static let AncientTextCulturalBridge = Persona(
