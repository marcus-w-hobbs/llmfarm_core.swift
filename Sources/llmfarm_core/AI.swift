...
public class AI {
  ...
  var aiQueue = DispatchQueue(
    label: "LLMFarm-Main", qos: .userInitiated, attributes: .concurrent,
  ...
  public func initModel(
    _ inference: ModelInference, contextParams: ModelAndContextParams = .default
  ) {
    self.model = nil
    switch inference {
    case .LLama_gguf:
  ...
  public func conversation(
    _ input: String, _ tokenCallback: ((String, Double) -> Void)?,
    _ completion: ((String) -> Void)?, system_prompt: String?, img_path: String? = nil
  ) {
    flagResponding = true
    aiQueue.async {
...
private typealias _ModelProgressCallback = (_ progress: Float, _ userData: UnsafeMutableRawPointer?)
  -> Void
...
public func get_model_sample_param_by_config(_ model_config: [String: AnyObject])
  -> ModelSampleParams
{
...
public func get_system_prompt(_ prompt_format_in: String) -> (String, String) {
  ...
  if prompt_format_in.contains(pattern) {
    ...
    prompt_format = String(prompt_format_in[end_i + 2..<prompt_format_in.count])
      .removingLeadingSpaces()
  }
  return (prompt_format, system_prompt)
}

public func get_model_context_param_by_config(_ model_config: [String: AnyObject])
  -> ModelAndContextParams
{
  ...
  if model_config["lora_adapters"] != nil {
    let tmp_adapters = model_config["lora_adapters"]! as? [[String: Any]]
    if tmp_adapters != nil {
  ...
  if model_config["clip_model"] != nil {
    tmp_param.clip_model = get_path_by_short_name(
      model_config["clip_model"]! as? String, dest: "models")
  }
  if model_config["reverse_prompt"] != nil {
    let splited_revrse_prompt = String(model_config["reverse_prompt"]! as! String).components(
      separatedBy: [","])
    ...
    && model_config["prompt_format"]! as! String != "{prompt}"
  {
    ...
    (tmp_param.custom_prompt_format, tmp_param.system_prompt) = get_system_prompt(
      tmp_param.custom_prompt_format)
...
public struct ModelAndContextParams {
  ...
  public var seed: UInt32 = 0xFFFF_FFFF  // RNG seed, 0 for random
