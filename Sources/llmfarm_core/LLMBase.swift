...
public class LLMBase /*: LLMInference*/ {
  ...
  public var modelLoadCompleteCallback: ((String) -> Void)? = nil
  ...
  public func llm_load_model(path: String = "", contextParams: ModelAndContextParams = .default)
    throws -> Bool
  {
    return false
  }
  ...
  public func predict(
    ...
    do {
      ...
      print(
        "Total tokens: \(inputTokensCount + output.count) (\(inputTokensCount) -> \(output.count))")
      // print("Past token count: \(nPast)/\(contextLength) (\(past.count))")
  ...
  public func llm_tokenize(_ input: String, add_bos: Bool? = nil, parse_special: Bool? = nil)
    -> [ModelToken]
  {
    return []
    // if input.count == 0 {
  ...
  public func tokenizePrompt(_ input: String, _ style: ModelPromptStyle) throws -> [ModelToken] {
    ...
    case .Custom:
      var formated_input = self.contextParams.custom_prompt_format.replacingOccurrences(
        of: "{{prompt}}", with: input)
