...
public class LLaMa_MModal: LLaMa {
  ...
  public override func load_clip_model() -> Bool {
    ...
    self.clip_ctx = clip_model_load(
      contextParams.clip_model, 1, contextParams.use_clip_metal ? 1 : 0)
    // #if os(iOS)
  ...
  public override func make_image_embed(_ image_path: String) -> Bool {
    self.image_embed = llava_image_embed_make_with_filename(
      self.clip_ctx, Int32(self.contextParams.n_threads), image_path)
    return true
  }

  private func retain_new_self_ptr() {
    LLaMaMM_obj = Unmanaged<LLaMa_MModal>.fromOpaque(Unmanaged.passRetained(self).toOpaque())
      .takeRetainedValue()
  }
